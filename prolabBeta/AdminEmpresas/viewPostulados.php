<?php
	...
	$oferta=$_GET["id"];
	$query ="SELECT eo.*, eo.muestraPostulantes as muestrapostu,eo.muestraDatosPersonales as muestradpers, e.nombrecomercial FROM empresas_ofertas eo, empresas e ";
	$query .="WHERE eo.id = '".$oferta."' and e.id=eo.id_empresa";
	$result =  $conexionDB->query($query);
	$row = $result -> fetchRow(DB_FETCHMODE_ASSOC);
    $campoMuestra = $row["muestrapostu"];
    $campoMuestraDPersonales = $row["muestradpers"];        
   ...
   if( $campoMuestra=="s"){ //muestra postulados
        $query ="SELECT DISTINCT pr.idU,ur.apellido,ur.nombre,ur.telefono, ur.email, ";
	...
	if($campoMuestraDPersonales=="s"){ //muestra datos personales postulados
	echo "<div class='element'>Haga click en Apellido o Nombres para ver CV</div>";
	echo ("<TR>");
	echo ("<th>   APELLIDO    </th>");
	echo ("<th>   NOMBRES     </th>");
	echo ("<th>TELEFONO/CELULAR</th>");
	echo ("<th WIDTH='20%'> MAIL </th>");
	//echo ("<th>  CELULAR  </th>");
	echo ("<th>NACIDO(edad)</th>");
	echo ("</TR>");
	...
	else{    /******************  MUESTRO SOLO LOS ID ***************/////
		echo "<div class='element'>Haga click en las CLAVES para ver CV</div>";
	    echo ("<TR>");
	    echo ("<th>   CLAVES IDENTIFICADORAS DE LOS USUARIOS    </th>");
	    echo ("</TR>");
	    echo ("<TR>");
	    echo ("<TD>");
		while ($rowPres = $postulados -> fetchRow(DB_FETCHMODE_ORDERED))
		{   
			echo("<a target='_blank' href='./AdminEmpresas/viewPostulado.php?id=".$oferta."&id_usuario=".$rowPres[0]."'>");
	        echo "<FONT COLOR='red'>".$rowPres[0]."</FONT></a> -  \n";
		}
	    echo ("</TD>");	    
		echo ("</TR>");
	}
