<?php
	...
	?>
	<input class='boton' type= 'submit' name='salir' value='Cerrar' type='button' onclick='window.close();'/>
	<BR/><BR/>
	<?php
	...
	while ($rowp = $postulados -> fetchRow(DB_FETCHMODE_ORDERED)){
		echo ("<a href='index.php?op=AdminEmpresas/viewPostulados&id=".$oferta."&id_usuario=".$rowp[1]."'>".$rowp[1]."</a>");
		echo (" | ");
	}
//*************************PRESELECCIONADOS****************************
	...
	echo ("<th>NOMBRES</th>");
	echo ("</TR>");
    $query ="SELECT DISTINCT pr.idUsuario,ur.apellido,ur.nombre FROM preseleccionados pr, usuarios ur";
	...
	while ($rowPres = $resultad -> fetchRow(DB_FETCHMODE_ORDERED)){
		echo ("<TR>");
		echo ("<TD>");
	    echo("<a href='index.php?op=AdminEmpresas/viewPostulados&id=".$oferta."&id_usuario=".$rowPres[0]."'>".$rowPres[1]."</a>  ");
	    echo ("</TD>");
	    echo ("<TD>");
	    echo("<a href='index.php?op=AdminEmpresas/viewPostulados&id=".$oferta."&id_usuario=".$rowPres[0]."'>".$rowPres[2]."</a>  ");
	    echo ("</TD>");
		echo ("</TR>");
    }
    echo ("</table>");
...
/********************************/
	...
	//********************OBJETIVOS********************************************************************/
		...
		$query ="SELECT uo.id,uo.oPersonales as person, uo.oLaborales as labor, uo.oProfesionales as profe FROM usuarios_objetivos uo ";
		$query .="WHERE uo.idUsuario =".$idUsuario."  order by uo.id desc";
		$result =  $conexionDB->query($query);
		if($ok = $result -> numRows()){		
			$row = $result -> fetchRow(DB_FETCHMODE_ASSOC);
			unset($data);
	...
	//********************EXPERIENCIA LABORAL**********************************************************/
		...
		$query .=" WHERE id_usuario = ".$idUsuario." ORDER BY ano_ingreso DESC, mes_ingreso DESC ";	
        $result =  $conexionDB->query($query);
		...
		/******************************************************************************************************/
				...
				$query ="SELECT ue.*,  (select re.descripcion from  rubro_empresa re where re.id=ue.id_rubro) as rubro_desc, (select ce.descripcion from  rubro_empresa re,categoria_empresa ce where re.id=ue.id_rubro and ce.id=re.id_categoria) as categoria  FROM usuarios_experiencia ue WHERE ue.id_usuario=".$idUsuario; 
        			$result =  $conexionDB->query($query);
	...
	?>
	<input class='boton' type= 'submit' name='salir' value='Cerrar' type='button' onclick='window.close();'/><BR><BR>
