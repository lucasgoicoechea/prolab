...
.label{
  ...
  if( isset($_SESSION['idUsr']) ){
	...
	$qUser =  $conexionDB->query("SELECT u.*,u.id_ciudad as city, cl.c_id as id_ciudad, cpa.c_id as id_partido, cpr.c_id as id_provincia, cl.d_descripcion as ciudad FROM usuarios u 
	inner join com_localidad cl on (cl.c_id = u.id_ciudad) 
	inner join com_partido cpa on (cpa.c_id=cl.c_id_partido) 
	inner join com_provincia cpr on (cpr.c_id = cpa.c_id_provincia)  where id=".$_SESSION['idUsr']);
    }
    else{
	$qUser =  $conexionDB->query("SELECT u.*,u.id_ciudad as city where id=".$_SESSION['idUsr']);
    }
  ...
  <table  width="100%" id="tablaPaso1">
	...
	if(!isset($_SESSION['idUsr'])){ //En el caso q se vaya a inserta agrego validaciones de usuario
		print "<tr><td class='label'>DNI</td><td><input id='dni' name='dni' type='text' class='caja' size='18' onkeyup='return autentica(this.value);'/>&nbsp;";
