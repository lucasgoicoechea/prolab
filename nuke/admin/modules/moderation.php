...
if ($row['radminsuper'] == 1) {
	...
	function moderation_news() {
		...
		while ($row = $db->sql_fetchrow($sql)) {
			echo "<td bgcolor=\"$bgcolor1\">&nbsp;<a href=\"".$admin_file.".php?op=moderation_news_view&id=$row[tid]\">$row[subject]</a>&nbsp;</td>";
			echo "<td bgcolor=\"$bgcolor1\">&nbsp;<a href=\"modules.php?name=Your_Account&op=userinfo&username=$row[name]\" target=\"_blank\">$row[name]</a>&nbsp;</td>";
			echo "<td bgcolor=\"$bgcolor1\" align=\"center\">&nbsp;<a href=\"".$admin_file.".php?op=moderation_approval&section=news&id=$row[tid]\"><img src=\"images/karma/approve.gif\" alt=\""._APPROVE."\" title=\""._APPROVE."\" width=\"15\" heigh=\"15\" border=\"0\"></a> &nbsp; <a href=\"".$admin_file.".php?op=moderation_reject&section=news&id=$row[tid]\"><img src=\"images/karma/reject.gif\" alt=\""._REJECT."\" title=\""._REJECT."\" width=\"15\" heigh=\"15\" border=\"0\"></a>&nbsp;</td></tr>";
			$a = 1;
		}
		if ($a != 1) {
	...
	function moderation_news_view($id) {
		...
		mod_menu();
		OpenTable();
		$comm = $db->sql_fetchrow($db->sql_query("SELECT * FROM ".$prefix."_comments_moderated WHERE tid='$id'"));
		$news = $db->sql_fetchrow($db->sql_query("SELECT title, hometext, bodytext, topic FROM ".$prefix."_stories WHERE sid='$comm[sid]'"));
	...
	function moderation_surveys_view($id) {
		...
		$poll = $db->sql_fetchrow($db->sql_query("SELECT pollTitle FROM ".$prefix."_poll_desc WHERE pollID='$comm[pollID]'"));
		if ($comm[pid] != 0) {
	...
	function moderation_reviews() {
		...
		while ($row = $db->sql_fetchrow($sql)) {
			...
			echo "<td bgcolor=\"$bgcolor1\" align=\"center\">&nbsp;<a href=\"".$admin_file.".php?op=moderation_approval&section=reviews&id=$row[cid]\"><img src=\"images/karma/approve.gif\" alt=\""._APPROVE."\" title=\""._APPROVE."\" width=\"15\" heigh=\"15\" border=\"0\"></a> &nbsp; <a href=\"".$admin_file.".php?op=moderation_reject&section=reviews&id=$row[cid]\"><img src=\"images/karma/reject.gif\" alt=\""._REJECT."\" title=\""._REJECT."\" width=\"15\" heigh=\"15\" border=\"0\"></a>&nbsp;</td></tr>";
			$a = 1;
		}
		if ($a != 1) {
			echo "<td bgcolor=\"$bgcolor1\" colspan=\"3\" align=\"center\">&nbsp;<i>"._NOCONTENT."</i>&nbsp;</td></tr>";
	...
	function moderation_reviews_view($id) {
		...
		$revi = $db->sql_fetchrow($db->sql_query("SELECT title FROM ".$prefix."_reviews WHERE id='$comm[rid]'"));
	...
	function moderation_approval($section, $id) {
		...
		if ($section == "news") {
			...
			$db->sql_query("INSERT INTO ".$prefix."_comments VALUES (NULL, '$row[pid]', '$row[sid]', '$row[date]', '$row[name]', '$row[email]', '$row[url]', '$row[host_name]', '$row[subject]', '$row[comment]', '$row[score]', '0', '0')");
			$db->sql_query("UPDATE ".$prefix."_stories SET comments=comments+1 WHERE sid='$row[sid]'");
		...
		if ($section == "surveys") {
			...
			$db->sql_query("INSERT INTO ".$prefix."_pollcomments VALUES (NULL, '$row[pid]', '$row[pollID]', '$row[date]', '$row[name]', '$row[email]', '$row[url]', '$row[host_name]', '$row[subject]', '$row[comment]', '$row[score]', '0', '0')");
			$db->sql_query("UPDATE ".$prefix."_poll_desc SET comments=comments+1 WHERE pollID='$row[pollID]'");
		...
		if ($section == "reviews") {
			...
			$db->sql_query("INSERT INTO ".$prefix."_reviews_comments VALUES (NULL, '$row[rid]', '$row[userid]', '$row[date]', '$row[comments]', '$row[score]')");
