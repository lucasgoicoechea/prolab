...
function main($user) {
	...
	if(!is_user($user)) {
		...
		if ($stop) {
			OpenTable();
			echo "<center><font class=\"title\"> NO ENTRA <b>"._LOGININCOR."</b></font></center>\n";
			CloseTable();
			echo "<br>\n";
		} else {
			OpenTable();
...
function login($username, $user_password, $redirect, $mode, $f, $t, $random_num, $gfx_check) {
	...
	if (($db->sql_numrows($result)==1) AND ($setinfo[user_id] != 1) AND ($setinfo[user_password] != "")) {
		...
		//cambiamos de base de graduados -> prolabDB
		$dbuname = "prolabunlp";
		$dbpass = "peReo61";
		$dbname = "graduados";
		...
		//if ($set == $username) {
			...
			//$db->sql_query("UPDATE aux SET usuario WHERE usuario='$username'");
			$sql = "SELECT usuario FROM aux WHERE usuario='$username'";
			$result = $db->sql_query($sql);
			$row = $db->sql_fetchrow($result);
			$dbuser = $row[usuario];
		//}
		...
		if ($dbuser != $username) {
			...
			Header("Location: modules.php?name=Login_Prolab");
			return;
		}
...
$user_prefix = "";
	...
	$sql     = "SELECT id, usuario FROM aux WHERE usuario='$username'";
	$result  = $db->sql_query($sql);
	$setinfo = $db->sql_fetchrow($result);
	$forward = ereg_replace("redirect=", "", "$redirect");
	if (ereg("privmsg", $forward)) {
		...
		if (extension_loaded("gd") AND $code != $gfx_check AND ($gfx_chk == 2 OR $gfx_chk == 4 OR $gfx_chk == 5 OR $gfx_chk == 7)) {
			...
			docookie($setinfo[id], $username, $new_pass, $setinfo[storynum], $setinfo[umode], $setinfo[uorder], $setinfo[thold], $setinfo[noscore], $setinfo[ublockon], $setinfo[theme], $setinfo[commentmax]);
			//docookie($setinfo[id], $username, 'repo', 0, 0, 0, 0, 0, 0, 0, 0);
			$uname = $_SERVER["REMOTE_ADDR"];
			$db->sql_query("DELETE FROM ".$prefix."_session WHERE uname='$uname' AND guest='1'");
			//$db->sql_query("UPDATE ".$prefix."_users SET last_ip='$uname' WHERE username='$username'");
		}
	if ($setinfo[usuario] == $username){
		if ($pm_login != "") {
			Header("Location: modules.php?name=Private_Messages&file=index&folder=inbox");
			exit;
		}
...
function edituser() {
	...
	if ((is_user($user)) AND (strtolower($userinfo['username']) == strtolower($cookie[1])) AND ($userinfo['user_password'] == $cookie[2])) {
		...
		."<td bgcolor='$bgcolor3'><textarea wrap=\"virtual\" cols=\"50\" rows=\"8\" name=\"user_sig\">$userinfo[user_sig]</textarea><br>"._255CHARMAX."</td></tr>"
		."<tr><td bgcolor='$bgcolor2'><b>"._EXTRAINFO.":</b><br>"._OPTIONAL."</td>"
		."<td bgcolor='$bgcolor3'><textarea wrap=\"virtual\" cols=\"50\" rows=\"8\" name=\"bio\">$userinfo[bio]</textarea><br>"._CANKNOWABOUT."</td></tr>"
...
function broadcast($the_message, $who) {
	...
	cookiedecode($user);
	getusrinfo($user);
	$row = $db->sql_fetchrow($db->sql_query("SELECT karma FROM ".$user_prefix."_users WHERE user_id='$cookie[0]'"));
	if ($row[karma == 2] AND $row[karma] == 3) {
		Header("Location: modules.php?name=$module_name");
		die();
	}
