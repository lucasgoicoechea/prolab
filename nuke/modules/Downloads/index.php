...
function menu($maindownload) {
    global $prefix, $user_adddownload, $module_name;
    OpenTable();
    $ThemeSel = get_theme();
...
function AddDownload() {
	...
	$result = $db->sql_query($sql);
    	...
    	echo "</select><br><br>"
    	    .""._LDESCRIPTION."<br><textarea name=\"description\" cols=\"70\" rows=\"15\"></textarea><br><br>"
...
function viewdownload($cid, $min, $orderby, $show) {
    global $prefix, $db, $admin, $perpage, $module_name, $user, $admin_file;
    include("header.php");
    if (!isset($min)) $min=0;
...
function viewdownloaddetails($lid, $ttitle) {
    ...
    $transfertitle = ereg_replace ("_", " ", $ttitle);
    $displaytitle = $transfertitle;
    $res = $db->sql_query("SELECT name, email, description, filesize, version, homepage FROM ".$prefix."_downloads_downloads WHERE lid='$lid'");
    list($auth_name, $email, $description, $filesize, $version, $homepage) = $db->sql_fetchrow($res);
	$auth_name = stripslashes($auth_name);
	$email = stripslashes($email);
    $description = stripslashes($description);
...
function brokendownloadS($lid, $modifysubmitter) {
    ...
    if (is_user($user)) {
		...
		$ratinguser = $cookie[1];
	    $lid = intval($lid);
		$db->sql_query("insert into ".$prefix."_downloads_modrequest values (NULL, '$lid', '0', '0', '', '', '', '$ratinguser', '1', '$auth_name', '$email', '$filesize', '$version', '$homepage')");
		include("header.php");
		menu(1);
		echo "<br>";
		OpenTable();
