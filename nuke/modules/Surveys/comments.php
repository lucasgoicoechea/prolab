...
function modthree($pollID, $mode, $order, $thold=0) {
	global $admin, $user, $moderate, $db, $prefix;
	$pollsid = intval($pollID);
	$comnum = $db->sql_numrows($db->sql_query("SELECT * FROM ".$prefix."_poll_desc WHERE pollID='$pollsid' AND comments!='0'"));
	if ($comnum != 0) {
		if((((isset($admin)) && ($moderate == 1)) || ($moderate==2)) && ($user)) {
			echo "<center><input type=hidden name=pollID value=$pollID>"
	    		...
	    		."<input type=hidden name=op value=moderate><br>";
	    	OpenTable();
...
function DisplayTopic ($pollID, $pid=0, $tid=0, $mode="thread", $order=0, $thold=0, $level=0, $nokids=0) {
	global $hr, $user, $datetime, $cookie, $mainfile, $admin, $commentlimit, $anonymous, $reasons, $anonpost, $foot1, $foot2, $foot3, $foot4, $prefix, $module_name, $db, $admin_file, $user_prefix;
	if($mainfile) {
...
function replyPreview ($pid, $pollID, $subject, $comment, $xanonpost, $mode, $order, $thold) {
	...
	echo"<font class=\"content\">"._HTMLNOTALLOWED."<br>";
	echo "<br>";
...
function CreateTopic ($xanonpost, $subject, $comment, $pid, $pollID, $host_name, $mode, $order, $thold) {
	global $user, $userinfo, $EditedMessage, $cookie, $prefix, $pollcomm, $anonpost, $db, $module_name, $user_prefix;
	...
	if ($fake == 1) {
		if ((($anonpost == 0) AND (is_user($user))) OR ($anonpost == 1)) {
			if (is_user($user)) {
				...
				if (isset($cookie[4])) { $koptions .= "&mode=$cookie[4]"; } else { $koptions .= "&mode=thread"; }
				if (isset($cookie[5])) { $koptions .= "&order=$cookie[5]"; } else { $koptions .= "&order=0"; }
				if (isset($cookie[6])) { $koptions .= "&thold=$cookie[6]"; } else { $koptions .= "&thold=0"; }
				if ($krow[karma] == 2) {
