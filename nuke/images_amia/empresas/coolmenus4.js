...
function cm_active(on,h){
  ...
  if(bw.reuse||bw.usedom){
    if(!this.img2) {if (this.cl2 != this.cl) this.o.evnt.className=on?this.cl2:this.cl;}
...
makeCM.prototype.makeMenu=function(name,parent,txt,lnk,targ,w,h,img1,img2,cl,cl2,align,rows,nolink,onclick,onmouseover,onmouseout){
  ...
  if(lnk && !onmouseover) m.mover="self.status='"+m.lnk+"'"
}
...
makeCM.prototype.hidesub=function(l,el){ //Changed v4.05
  ...
  for(i=l-1;i<c.l.length;i++){
    ...
    if(i>l){for(j=0;j<c.l[i-1].arnum;j++)
	{
		//Me fijo si es objeto o str, si es objeto llamo al metodo hideIt de la flechita.
		if (!c.l[i-1].arr[j].length)
		{
